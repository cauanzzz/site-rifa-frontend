import { ChevronDown, Coins, Info, LifeBuoy, Menu, Ticket, X } from "lucide-react";
        ...
        <>
            <header className="bg-white shadow-sm border-b">
                <div className="container mx-auto px-4 py-4 md:py-6">
                    ...
                    {menuMobileAberto && (
                        <div className="lg:hidden mt-4 pt-4 border-t flex flex-col gap-2 animate-in slide-in-from-top-2">
                            {logado && (
                            ...
                            <Link to="/about-us" className="flex items-center gap-3 px-2 py-2 text-gray-700" onClick={() => setMenuMobileAberto(false)}>
                                <Info className="w-5 h-5 text-blue-500" /> Sobre nós
                            </Link>
                            <Link to="/comprar-moedas" className="flex items-center gap-3 px-2 py-2 text-gray-700" onClick={() => setMenuMobileAberto(false)}>
                                <Coins className="w-5 h-5 text-yellow-500" /> Comprar moedas
                            </Link>
                            <Link to="/suporte" className="flex items-center gap-3 px-2 py-2 text-gray-700" onClick={() => setMenuMobileAberto(false)}>
                                <LifeBuoy className="w-5 h-5 text-green-500" /> Suporte
                            </Link>
                            ...
                            <div className="mt-4 pt-4 border-t flex flex-col gap-3">
                                {logado ? (
                                    <>
                                        {isAdmin && (
                                            <Link to="/admin" onClick={() => setMenuMobileAberto(false)}>
                                                <Button variant="outline" className="w-full justify-start border-purple-200 text-purple-700">
                                                    👑 Painel Admin
                                                </Button>
                                            </Link>
                                        )}
                                        <Link to="/minhas-rifas" onClick={() => setMenuMobileAberto(false)}>
                                            <Button className="w-full justify-start bg-gradient-to-r from-purple-600 to-blue-600 text-white">Minhas Rifas</Button>
                                        </Link>
                                        <Link to="/criar-rifa" onClick={() => setMenuMobileAberto(false)}>
                                            <Button className="w-full justify-start bg-gradient-to-r from-purple-600 to-blue-600 text-white">Criar Rifa</Button>
                                        </Link>
                                        <Button variant="outline" className="w-full justify-start" onClick={fazerLogout}>Sair</Button>
                                    </>
                                ) : (
                                    <>
                                        <Button variant="outline" className="w-full border-purple-600 text-purple-600" onClick={() => { setMostrarCadastro(true); setMenuMobileAberto(false); }}>Cadastrar-se</Button>
                                        <Button className="w-full bg-gradient-to-r from-purple-600 to-blue-600 text-white" onClick={() => { setMostrarLogin(true); setMenuMobileAberto(false); }}>Login</Button>
                                    </>
                                )}
                            </div>
                        </div>
            ...
            {mostrarLogin && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
                    <div className="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-md relative mx-4">
                        <button 
